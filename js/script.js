...
function renderNotes(notes) {
    ...
    // 预定义的排序顺序
    const customOrder = [
        "1起", "点券10起", "点券10可限", "qb10起", "qb10可限",
        "点券50起", "qb50起", "点券100", "点券100极速", "qb100起",
        "单笔200", "单笔200极速", "点券可限", "可限极速", "心悦卡100/200"
    ];

    // 解析折扣行并映射到对象
    const discountMap = {};
    const allDiscounts = []; // 保存所有折扣项
    ...
    notes.forEach(note => {
        ...
        if (match) {
            ...
            discountMap[prefix] = discount;
            allDiscounts.push(prefix); // 保存原始顺序
        }
    });
    ...
    // 按自定义顺序渲染
    customOrder.forEach(prefix => {
        if (discountMap.hasOwnProperty(prefix)) {
            const noteItem = document.createElement('div');
            noteItem.className = 'note-item';
            noteItem.innerHTML = `${prefix} <strong>${discountMap[prefix]}</strong>`;
            container.appendChild(noteItem);
            renderedItems.add(prefix);
        }
    });

    // 添加分隔线（如果有额外折扣）
    if (allDiscounts.some(prefix => !renderedItems.has(prefix))) {
    ...
    // 渲染不在自定义顺序中的折扣（按原始顺序）
    allDiscounts.forEach(prefix => {
        if (!renderedItems.has(prefix) && discountMap.hasOwnProperty(prefix)) {
            const noteItem = document.createElement('div');
