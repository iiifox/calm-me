...
// @updateURL    https://iiifox.me/assets/giraffe/login/pizi1.js
// @downloadURL  https://iiifox.me/assets/giraffe/login/pizi1.js
// ==/UserScript==
...
(function () {
    ...
    function checkAndSetSecret() {
        let secret = GM_getValue('pizi1_totp_secret', null);

        if (!secret) {
            secret = prompt('请输入你的TOTP密钥（Base32格式）：', '');
            if (secret && secret.trim() !== '') {
                GM_setValue('pizi1_totp_secret', secret.trim());
                alert('密钥已保存');
                return secret.trim();
            } else {
    ...
    GM_registerMenuCommand('重新设置TOTP密钥', () => {
        ...
        if (newSecret && newSecret.trim() !== '') {
            GM_setValue('pizi1_totp_secret', newSecret.trim());
            alert('密钥已更新');
            location.reload();
        }
    });
