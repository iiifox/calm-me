...
// 将 /price.txt 解析为 JSON：{ date, data(兼容), qz, gbo, yesterday_page }
...
function formatRateValue(value) {
  ...
  if (num === 0) return 0;
  if (num < 2) return num;
  if (num < 20) return +(num / 10).toFixed(3);    // 8.2 -> 0.82?（按你现有逻辑映射区间）
  if (num < 200) return +(num / 100).toFixed(3);  // 75 -> 0.75
  return +(num / 1000).toFixed(3);                // 915 -> 0.915 / 865 -> 0.865
}
...
export async function onRequest() {
  ...
  if (!resp.ok) {
    return new Response(JSON.stringify({ error: 'price.txt 获取失败' }), { status: 502, headers: { 'Content-Type': 'application/json' } });
  }
    ...
    if (inWechat) {
      ...
      if (m) {
        ...
        if (v) {
          ...
          if (prefix) {
            prefix.split(/[、，,]/).map(s => s.trim()).filter(Boolean).forEach(k => { gbo[k] = val; });
          }
        }
      }
      continue;
