...
- ABCDE分别为不同的价格排序越后价格越贵速度越快（回调 100-2000 之间整百金额）
...
- 安卓QQ区：上号 + 抓包传码（饭票）
- 安卓微信区：PC通用，游戏随便选，链接用苹果新快手
- 苹果QQ区：正常挂，随缘进
- 苹果微信区：PC通用，游戏随便选，链接用苹果新快手

### 无畏契约

心悦无畏 —— 预产 —— 开启钱包直冲，开启拉单

最近风控比较严，心悦无畏容易出验证码，而PC端又普遍不出验证码，酌情考虑是否换无畏契约通道（无畏契约1000的话实付只有988，心悦无畏可以998）
...
```
https://iiifox.me/js/gbo/enableGbo.js
```
...
```
http://116.62.60.127:8369/Webindex.aspx
```

个人设置当中绑定 Google Authenticator
...
- 所选业务：单笔金额500以上用王者4笔Q钱点卷（电商），100-300用王者Q钱点卷（电商）。如果直冲则随意
- 实付金额：慢充总金额 不是单笔（直冲填1就行）
- 链接数：字面意思
- 所选返利：不填
- 备注：字面意思
...
录入成功后默认是启用，即默认是慢充的费率，需要直冲的话需要把状态切至停用

状态：已启用表示台慢充；已停用表示直冲。

直冲流程：你需要复制2级户账号信息中的淘宝订单，然后去总台账号信息搜索这个淘宝订单，选择最左边的生成直冲链接。

目前直冲只有 **王者Q钱点券(电商)** 渠道的可正常结算

> 生成直冲链接注意点：
> 
> 支付金额为单笔金额，即客户充2000，你可以拆分为4个500，但是拆分的话要前一笔到账才能继续生成，因为只能看到一个链接
> 
